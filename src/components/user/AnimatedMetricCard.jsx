...
const AnimatedMetricCard = ({ title, value, subtitle, icon: Icon, colorTheme = 'blue', delay = 0 }) => {
  const [displayVal, setDisplayVal] = useState(0);

  // Extract numeric part of value if possible (e.g. "85/100" -> 85, "120 XP" -> 120, "#4" -> 4)
  ...
  useEffect(() => {
    ...
    const animate = (currentTime) => {
      ...
      const currentVal = Math.round(start + easeOutQuad(progress) * (target - start));
  ...
  // Helper to re-assemble the formatted string
  const getFormattedValue = () => {
    const str = String(value);
    if (str.includes('/')) {
      return `${displayVal}/${str.split('/')[1]}`;
    }
    if (str.includes('XP')) {
      return `${displayVal} XP`;
    }
    if (str.startsWith('#')) {
      return `#${displayVal}`;
    }
